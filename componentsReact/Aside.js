      ...
      <div className="add_note_div">
        ...
        <ul className="FileList" id="FileList">
          {noteLiPages.map((noteLiPage) => (
            <li
              key={noteLiPage.id}
              ref={(current) => (liRefs.current[noteLiPage.value] = current)}
              onClick={() => addNotepage(noteLiPage)}
            >
              {noteLiPage.value}
              <button
                id="liClose"
        ...
        <div id="asideModalWindow" tabIndex="-1">
          <div id="ModalWindow">
            <input
              ...
              id="modalWindowInput"
              type="text"
              placeholder=" Enter new note page name..."
              ref={inputRef}
              onKeyDown={(event) => {
                if (event.key == "Enter") {
                  addNoteLiPage();
            ...
            ></input>
            <button id="modalWindowButton" onClick={addNoteLiPage}>
              Submit
            </button>
          </div>
        </div>
