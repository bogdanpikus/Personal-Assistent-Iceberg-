...
export default function WeatherBlock() {
    ...
    const toggleWeatherBlock = () => {
        if (weatherBlockRef.current) {
            ...
            weatherBlockRef.current.style.display = currentDisplay === 'none' || currentDisplay === '' ? 'inline-block' : 'none';
        }
    };
        ...
        <>
            <article className="weather_block" id="weather_block">
                <div className="weather_button_div">
                    <button className="weather_button" id="weather_id_button" onClick={toggleWeatherBlock}>
                        <span>Weather</span>
                    </button>
                </div>

                <div className="weather_block_div" id="weather_block_div" ref={weatherBlockRef}>
                    {weather ? (
                        <>
                            <button onClick={openCountryList} className="button_ChangeWeatherTown">
                                <span className="span_WeatherTownChange">...</span>
                            </button>
                            <p>{weather.city}</p>
                            <h1>{weather.temp}°C <small>Feels like: {weather.feelsLike}°C</small></h1>
            ...
            <div id="cityChangeBlock" ref={countyChangeBlockRef}>
                {showCountryList && (
                    <div className="choseContry" id="choseContry">
                        <ul className="choseContry_ul" id="choseContry_ul" onClick={() => setShowCountryItems(current => !current)}><span>Chose your contry...</span>
                            {showCountryItems && (
                                <li className="choseContry_li" id="choseContry_li" onClick={openCityList} >Ukraine
                                    <Image
