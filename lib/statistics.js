...
export default function Table() {
    ...
    useEffect(() => {
        updateStats(dateCellMap);
    }, [dateCellMap]);

    function updateStats(dateCellMap) {
        if (!dateCellMap.size) return;
        ...
        openDB().then(db => {
            ...
            notesReq.onsuccess = () => {
                ...
                const hasChecked = notes.some(note => note.checked === true);

                if (hasChecked) {
                    const statsTx = db.transaction("Stats", "readwrite");
                    const statsStore = statsTx.objectStore("Stats");
                    const statEntry = { id: today, status: "done" };
                    statsStore.put(statEntry);
                }

                // Вызывается всегда, даже если нет "checked"
                highlightStats(db, dateCellMap);
            };

            notesReq.onerror = () => console.error("Notes read error", notesReq.error);
        });
    }

    function highlightStats(db, dateCellMap) {
        const statsTx = db.transaction("Stats", "readonly");
        const statsStore = statsTx.objectStore("Stats");
        const statsReq = statsStore.getAll();

        statsReq.onsuccess = () => {
            const stats = statsReq.result;
            stats.forEach(stat => {
                if (stat?.id && dateCellMap.has(stat.id)) {
                    const td = dateCellMap.get(stat.id);
                    td.classList.add("highlighted-today");
                }
            });
        };

        statsReq.onerror = () => console.error("Stats read error", statsReq.error);
    }
