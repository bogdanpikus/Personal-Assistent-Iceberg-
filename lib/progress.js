            ...
            {showModalWindow && (
                <div className="modal_overlay" id="modal_overlay" style={{ display: 'flex' }}>
                    <div className="modal_window" id="modal_window_Div">
                        <h2>Новая Задача</h2>
                        <input type="text" id="note_input" placeholder="Введите задачу..." ref={inputRef} onKeyDown={(event) => { if (event.key === 'Escape') { closeModalWindow() } if (event.key === 'Enter') { saveNoteInEndexedDBAndShowItIn_task_item() } }} />
